...
class _HomePageState extends State<HomePage> {
  ...
  void _setupTripNotifications() {
    ...
    _notificationService.onTripStartReceived = (TripNotificationModel trip) {
      ...
      setState(() {
        ...
        debugPrint('✅ Trip added to activeTrips. Total trips: ${activeTrips.length}');
      });
      
      // Show notification
      final displayName = trip.travelerName.isNotEmpty ? trip.travelerName : 'Unknown Traveler';
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
    ...
    getIt<SignalRService>().tripEventsStream.listen((event) {
      ...
      if (event['event'] == 'TripEnded') {
        ...
        final tripId = data['TripId']?.toString() ?? data['tripId']?.toString() ?? '';
        ...
        setState(() {
          ...
          debugPrint('✅ Trip removed from activeTrips. Before: $initialCount, After: $finalCount');
        });
        
        // Show notification
        final travelerName = data['TravelerName']?.toString() ?? data['travelerName']?.toString() ?? 'Traveler';
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            ...activeTrips.map((trip) {
              ...
              return Padding(
                ...
                child: MapCard(
                  ...
                  date: displayDate,
                  time: displayTime,
                  address: 'Trip in progress', // You can update this with actual location
                  isActive: true,
                  onMapTap: onMapTap,
                  tripId: trip.tripId,
                  onRemove: () {
  ...
  String _formatTime(String startTime) {
    ...
    try {
      final date = DateTime.parse(startTime);
      final result = '${date.hour.toString().padLeft(2, '0')}:${date.minute.toString().padLeft(2, '0')}';
